...
for day in days:
    ...
    load_data_path = os.path.dirname(os.path.dirname(__file__)) + "/data/load-data_Hourly"
    ...
    Results.initialise(time_settings, end_buses, lines_ratings, None, None, step_size=step_size)
    ...
    Results.export_metrics(path=os.path.dirname(__file__) + '/results', file_name=f'baseline_metrics_hourly_{day}')
    Results.export_voltages(path=os.path.dirname(__file__) + '/results', file_name=f'baseline_voltages_hourly_{day}')
    Results.export_line_currents(path=os.path.dirname(__file__) + '/results', file_name=f'baseline_line_currents_hourly_{day}')
    Results.export_voltage_unbalance(path=os.path.dirname(__file__) + '/results', file_name=f'baseline_voltage_unbalance_hourly_{day}')
    Plots.plot_and_save_all(os.path.dirname(__file__) + f'/plots/baseline/{day}/', extra_disc='_baseline_hourly_' + day)
