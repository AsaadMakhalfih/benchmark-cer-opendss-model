...
from src.models.inverter import Inverter, HybridInverter, InverterSettings, VoltWatt, StaticExportLimit, HybridInverterSettings, MaximiseSelfConsumptionSettings
...
for day in days:
    ...
    delta_p_q_settings = (0.175, 0.085, 0.122, 0.05, 0.175, 0.07, 0.12, 0.05)
    ...
    Results.update_simulation_time(t2 - t1)
        # print('time:', step / 2.0, 'soc:', batteries[3].soc)
    ...
    case_name = f'load_pv_battery_self_consumption_export_limit_hourly_{day}'
    Results.export_summary_results(path=os.path.dirname(__file__) + '/results', file_name=case_name)
    Results.export_energy_flow_results(path=os.path.dirname(__file__) + '/results', file_name=f'load_pv_battery_self_consumption_export_limit_energy_flow_hourly_{day}')
    case_name = f'load_pv_battery_self_consumption_export_limit_hourly'
